...
export function login(email, password) {
    return axios.post("https://kodbazis.hu/api/login-user", { email, password }, 
...
export function logout() {
    return axios.post("https://kodbazis.hu/api/logout-user", {}, 
    { withCredentials: true })
    .then((res) => {
        accessToken = "";
    });
...
fetchHitelesitessel.interceptors.response.use(
    ...
    (error) => {
        ...
        return axios.get("https://kodbazis.hu/api/get-new-access-token", 
        { withCredentials: true })
        .then((res) => {
            accessToken = res.data.accessToken;
        })
